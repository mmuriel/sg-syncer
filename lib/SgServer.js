...
var SgServer = (function(socketServer,wtClient,filesWatcher,sgCnf,httpServer,db){
	...
	var _startToListenSocketsEvents = function(){
		...
		socketServer.sockets.on('connection', function(socket){
		    ...
		    socket.on('get last torrents',function(data){

		        SgDb.select("torrents","hashid,created_at,magnetURI,filename",` date_created >='${data.date}' `)
		        .then(function(data){
		        	//Gestiona el exito de la busqueda
		        	socket.emmit("last torrents",data);
		        },function(data){
